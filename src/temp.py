...
def calculate_fitness(agents,trainLoader):
    # for agent in agents:
...
#     offSpring = []
#     for index,agent in enumerate(agents):
#         if random.random()>=(1-index/len(agents))**2:
#             child=Agent(featureDim=featureDim)
...
def run_genetic_algorithm(modelName,modelVersion,batchSize):
    ...
    for i in range(generations):
        agents=calculate_fitness(agents,trainLoader=trainLoader)
        # print(type(agents))
...
if __name__=="__main__":
    ...
    # run_genetic_algorithm(modelName='LSTM',modelVersion='7',batchSize=512)
    print("All is well!")
    writer.close()
